...
running `Three_TC.train` per point, minus the per-point process spawn.
...
(re)initialise the weights, and hand the reused `vs` to `Three_TC.train.train`
...
Usage (CLI / cluster) — one chunk of the hz phase sweep at L=4:
    python -m Three_TC.sweep --L 4 --bc OBC --model bosonic --arch ToricCNN_gridinv \
...
from Three_TC.builders import build_state, build_hamiltonian, with_defaults
...
def _parse_args() -> Dict[str, Any]:
    ...
    p = argparse.ArgumentParser(
        description="Batch a chunk of field points through one process "
                    ...
                    "TRAIN_DEFAULTS / builders.DEFAULTS, exactly like Three_TC.train.")
    # Sweep control
