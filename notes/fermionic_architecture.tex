...
certificates. The amplitude ladder then hit every exact anchor:
$\delta = 5.3\times10^{-8}$ ($L{=}4$), $3.5\times10^{-8}$ ($L{=}5$),
$7.1\times10^{-7}$ ($L{=}6$, sampling-resolution-limited at $4096$ samples,
stopped after 3 steps) — Sec.~\ref{sec:status}.
...
\label{eq:tokens}
\end{equation}
the plaquette-boundary $z$-parities of a configuration $s$. In the $z$-basis
the stabilizer eigenvalue equations $\Av\ket\psi=\ket\psi$,
$\Bt\ket\psi=\ket\psi$ become \emph{two-amplitude relations}. To see this,
expand $\ket\psi=\sum_s\psi(s)\ket s$ and act with one stabilizer on a basis
state. The decoration pair $d_p=\{e_+,e_-\}$ sits on edges perpendicular to
the plaquette plane, hence disjoint from $\partial p$, so
\[
\Bt\ket s \;=\; Z(\partial p)\,\sx_{e_+}\sx_{e_-}\ket s
\;=\; Z(\partial p)\ket{s\oplus d_p}
\;=\; \varepsilon_p(s)\ket{s\oplus d_p} :
\]
the $\sx$ pair flips the two decoration edges, and the diagonal
$Z(\partial p)$ then reads off the plaquette parity, which the flip did not
touch. Substituting into $\Bt\ket\psi=\ket\psi$, relabeling the summation
variable $s\to s\oplus d_p$, and matching the coefficient of each $\ket s$ on
both sides forces $\varepsilon_p(s)\,\psi(s\oplus d_p)=\psi(s)$, i.e.\ one
relation per (stabilizer, configuration) pair:
\begin{equation}
...
\end{equation}
the star relation carrying no sign factor ($\Av$ is a pure $\sx$ product).
Propagating \eqref{eq:propagation} breadth-first from the seed
$\psi(0\cdots0)=1$ enumerates the ground-state support with exact signs:
each application hands you a new configuration together with its sign
relative to the seed (path-independent, since the stabilizers commute), and
configurations unreachable by flip moves have $\psi=0$ (at $L=2$: $2^{15}$
support states, 56\%/44\% positive/negative). Structurally, sign-changing
moves (decoration-pair flips
...
token vector}, and --- as derived below --- that function is a $\GF$
quadratic form: on the support,
\begin{equation}
...
\paragraph{Why quadratic, and what $\ell$ and $Q$ are.}
In \eqref{eq:quadform} the token vector is read as bits ($t_p=1$ meaning
flux $-1$), $\ell\in\GF^{N_p}$ collects the coefficients of the linear part,
and $Q$ is an upper-triangular $\GF$ matrix of pairwise couplings --- so the
claim is that the sign is a product of single-token and token-\emph{pair}
factors, nothing higher-order. This needs no appeal to authority: it follows
from the propagation rule \eqref{eq:propagation} by counting. Build a
support state by applying a subset $x\in\GF^{N_p}$ of the decorated
plaquette operators to $\ket{0\cdots0}$ (stars contribute no sign and may be
ignored). Each application of $\tilde B_q$ contributes $\varepsilon_q$ evaluated
on the state built so far; starting from all-up, where every
$\varepsilon_q=+1$, that parity has been toggled exactly once by each
\emph{previously applied} pair $d_p$ with $|\partial q\cap d_p|$ odd. The
total sign therefore counts \emph{pairwise collisions} between flip
operations,
\begin{equation}
...
\label{eq:Cform}
\end{equation}
quadratic because collisions come in pairs: flip $p$ perturbs the diagonal
factor of flip $q$ one pair at a time, and no mechanism generates a
three-flip term. (Minimal case: apply $p$ then $q$ on the seed; the sign is
$\varepsilon_q(0\cdots0\oplus d_p)=(-1)^{C_{qp}}$, a single quadratic
monomial. $C$ is symmetric, so the exponent is independent of application
order, as commutativity demands.) The tokens, meanwhile, are \emph{linear}
in $x$ --- flipping $d_p$ toggles token $q$ iff $C_{qp}=1$, so $t=Cx$ --- and
substituting a linear right-inverse $x(t)$ into \eqref{eq:Cform} gives
exactly the form \eqref{eq:quadform}: products of linear forms stay
quadratic, and over $\GF$ squares collapse to linear terms ($a^2=a$), which
is where $\ell$ comes from. That substitution is performed explicitly in
Sec.~\ref{sec:pullback} (the ``C-form pullback''). Independently, the
canonical-form theorem for stabilizer states \cite{DehaeneDeMoor2003}
guarantees \emph{some} $\GF$ quadratic sign structure; and
Sec.~\ref{sec:sampled} verifies the token-quadratic hypothesis wholesale ---
the equation systems for $(\ell,Q)$ saturate at exactly $d(d+1)/2$ unknowns
with zero contradictions over thousands of sampled equations, where a single
higher-order effect would have produced inconsistent rows.
...
\file{analysis/notebooks/fermionic\_h0\_prefit\_ladder.ipynb} for the curves;
\file{analysis/scripts/prefit\_phase\_head.py} is the tool.
...
The replacement (commit \texttt{ff3d1c0}, \file{analysis/scripts/prefit\_phase\_head.py})
...
rules \eqref{eq:propagation} (commutation makes the order irrelevant, and
stars are provably irrelevant --- they change neither tokens nor signs). Each draw yields one $\GF$ equation for $(\ell,Q)$; an
...
\file{analysis/scripts/stencil\_phase\_head.py}, commit \texttt{8b4ec59}.)

\paragraph{Where the sign \emph{is} local and TI: application variables.}
In application variables the answer is already in hand: the accumulated sign
is the collision form \eqref{eq:Cform} derived in Sec.~\ref{sec:start},
$\mathrm{sign}(x)=(-1)^{\sum_{p<q}C_{pq}x_px_q}$ --- analytic, requiring no
solving at all, with $C$ symmetric (commutation makes odd overlap
reciprocal), range exactly one lattice unit, and translation invariant: each
plaquette couples to exactly 8 neighbors. Verified by sampling, 3000/3000 at
$L=2,3,4$. This is the operational, bosonization-phase form of the
sign.\footnote{A pleasing coincidence of the geometry: the same $\GF$
...
\texttt{--analytic\_C} in \file{analysis/scripts/prefit\_phase\_head.py}; stored
...
and \texttt{--chains\_up}, amplitude-only SR hit every exact anchor
$E_0=-4L^3$ (all runs early-stopped at the sampling-resolution floor —
user-approved compute hygiene; $L{=}6$ starts at $\delta=4.9\times10^{-6}$
from the \emph{cold} trunk and needed only 3 SR steps):
...
final $E$ & $-255.999986(17)$ & $-500.000017(122)$ & $-863.99939(32)$\\
$\delta$, Vscore & $5.3{\times}10^{-8}$, $7{\times}10^{-9}$
  & $3.5{\times}10^{-8}$, $1.0{\times}10^{-7}$
  & $7.1{\times}10^{-7}$, $3.7{\times}10^{-7}$\\
\bottomrule
\end{tabular}
\end{center}
...
\paragraph{Frozen does not mean rigid (a worry worth retiring).}
A natural objection: configurations with zero amplitude at $h=0$ acquire
weight at $h\neq0$ and the phases deform --- does a frozen head not prevent
the network from adapting continuously? No. The total phase is (frozen head)
$+$ (trainable trunk phase) and the amplitudes are trunk-owned, so the
ansatz remains a continuously deformable family \emph{around} the pinned
reference point; freezing removes no directions from the smooth channel.
(The head is a form on \emph{all} of token space, so newly reachable
configurations receive a well-defined reference phase too; and the penalty
is soft --- finite $\kappa$ suppresses, not zeroes, off-sector weight.) Nor
would \emph{learning} the $h=0$ sign structure buy any continuity that
installing it does not: a learned representation lives in the same function
space and admits the same deformations, differing only in that it starts
with errors --- each a permanent $O(1)$ energy tax
(Sec.~\ref{sec:options}), stored in a form that drifts under early amplitude
work --- and a local TI trunk asked to carry the discrete part is fighting
the obstruction of Sec.~\ref{sec:stencil} outright at even $L$. In one
line: \emph{at $h=0$ the discrete channels are the answer; at $h\neq0$ they
become the coordinate system the optimizer deforms around.}
...
low-rank --- so the discrete data remains untouchable. Zero-init is safe
here, unlike in Sec.~\ref{sec:options}(c): the $b\equiv0$ saddle existed
because every sign loss is even under flipping all phases, and the frozen
head already breaks that symmetry --- at zero correction the total phase
sits at a generic, non-symmetric point, so gradients flow. Whether such a
head is ever needed is an empirical question for the first $L=2$
ED-benchmarked sweeps.

\paragraph{An observation worth pinning down first (peer-verified
2026-08-11; one gap open).}
...
\emph{Status update (2026-08-11).} An adversarial review by the
fermionic-ladder session confirmed both halves. Superselection is
unconditional: the $u_c$ commute term-by-term with $H$ at $h_x=0$ (even star
overlap with any boundary XOR; $M^{\!\top}c=0$ is the defining equation of
the masks). The rotated-frame stoquasticity also closes: star matrix
elements carry no residual head phase because star flips are
token-invariant, the identity $\varepsilon_p(s)=t_p(s)$ makes the $\Bt$
cancellation manifest, and Perron--Frobenius then fixes the \emph{sector}
ground state's signs to the head's at every $h_z$. The single remaining gap
is whether the \emph{global} ground state stays in the $u\equiv+1$ sector at
intermediate $h_z$ --- both endpoints are safe ($h=0$ by construction;
the $h_z\to\infty$ polarized state has $t\equiv+1\in V$), and flux
excitations cost $\Bt$ energy while gaining nothing from a flux-neutral
diagonal field, so the expectation is yes --- but it is not excluded by
generalities. That gap is exactly what the sector-resolved $L=2$ ED check of
Sec.~\ref{sec:validation} closes.
...
conditioning even where it is not strictly needed for correctness. On the
electric line this is not optional (2026-08-11 review): sector conservation
is a property of $H$, not of the variational state --- the token-blind trunk
would leak ghost weight exactly as it did at $h=0$ --- so the penalty stays
on there, exact, with $\kappa=6$ frozen.
...
calibrate the $\kappa$ bias and any trunk-phase drift. On the electric line
the protocol is fixed (2026-08-11): per $h_z$ on a grid over $[0,\sim0.6]$,
extract (i) the \emph{sector-resolved} ground energy ($u\equiv+1$ vs.\
lowest other sector) --- closing the stability gap of
Sec.~\ref{sec:finitefield}; (ii) the phase audit
$\psi_{\mathrm{ED}}(s)\,(-1)^{q(t(s))}>0$ on the support --- an end-to-end
test of the rotated-frame claim, including convention slips; (iii) the
$E(h_z)$ curve itself, the first exact finite-field benchmark for the NQS
electric sweeps. Gate \emph{trust} on the ED; build sweep infrastructure in
parallel.
\item \textbf{Low-field perturbative series} \`a la
\file{analysis/scripts/exact\_benchmarks.py}, extended to the fTC --- the same
...
the new failure modes) live? \emph{[Settled 2026-08-11: electric first,
magnetic second --- see the status update in Sec.~\ref{sec:finitefield}.]}
...
\emph{[Algebra verified 2026-08-11; the sector-stability question is the one
open gap, closed by the ED protocol of Sec.~\ref{sec:validation}.]}
...
\emph{Clifford group, stabilizer states, and linear and quadratic operations
over GF(2)}, Phys.\ Rev.\ A \textbf{68}, 042318 (2003),
arXiv:quant-ph/0304125.
