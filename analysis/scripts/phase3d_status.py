...
# shown in the viewer right after each cut's learning-curve panel. Keyed by {hy_str: {cut_id:
# {"status", "comment"}}}; hand-edited (or agent-edited) JSON, NOT regenerated by this script --
# degrades to "no entry" (viewer shows nothing) on a missing/malformed file, same convention as
# the rest of export_viewer.
CUT_STATUS_FILE = Path(__file__).parent.parent / "viewer" / "cut_status.json"


def load_cut_status(path=CUT_STATUS_FILE) -> dict:
    try:
...
ELECTRIC_FIRST_ORDER = {(0.4, 0.8), (0.6, 0.8), (0.8, 0.65), (0.8, 0.8)}   # (hy, hx); at hy=0.8 the lobe ends near hx 0.6
...
def export_viewer(root, curves_root, hy, min_points=5, tol=1e-9) -> dict:
    ...
    cut_status = load_cut_status().get(str(hy), {})   # matches the "hy0.0"/"hy0.2"/"hy0.4" convention
    ...
    if len(df):
        for (cut, ffield, fval), g in df.groupby(["cut", "fixed_field", "fixed_val"]):
            ...
            cut_dict = {"id": cut_id, "kind": "electric" if cut == "electric" else "first-order",
