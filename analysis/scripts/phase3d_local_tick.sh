...
ssh -o ConnectTimeout=30 perlmutter 'q=$(squeue -u sanzharb -h -o "%T %j" | grep -v "p3d-driver\|p3d-wandb"); echo "WATCH $(date +%H:%M): running=$(echo "$q" | grep -c ^RUNNING) pending=$(echo "$q" | grep -c ^PENDING) | failed_last6h=$(sacct -X -u sanzharb -S $(date -d "6 hours ago" +%Y-%m-%dT%H:%M) -s F,TO,OOM,NF -n -o JobID,JobName%22,State | grep -v "p3d-driver\|p3d-wandb" | awk "{print \$1\":\"\$2\":\"\$3}" | tr "\n" " ")"; find ~/toric-code-nqs -maxdepth 2 -name "p3d_*.out" -mmin -360 2>/dev/null | xargs -r grep -l "GENUINE DIVERGENCE\|CHAIN STOPPED" 2>/dev/null | xargs -r -n1 basename | sed "s/^/  DIV /"; find ~/toric-code-nqs -maxdepth 2 -name "p3d_*.out" -mmin -360 2>/dev/null | xargs -r grep -l "srun: error:.*Exited with exit code" 2>/dev/null | xargs -r -n1 basename | sed "s/^/  CRASH (step died, batch may still say COMPLETED) /"; for hy in y; do f=$PSCRATCH/tc_nqs/phase3d/driver_hy$hy.log; [ -f $f ] && echo "  driver hy$hy: $(grep "\[launch\].*job(s)\|\[watch\] flagged" $f | tail -2 | tr "\n" " ")"; done; s=""; for j in $(squeue -u sanzharb -h -t PD -o "%A %j %l" | awk "\$2 ~ /^p3d_/ && \$2 !~ /driver/ && \$3 != \"2:00:00\" {print \$1}"); do scontrol update JobId=$j TimeLimit=2:00:00 2>/dev/null && s="$s $j"; done; [ -n "$s" ] && echo "  SHORTENED to 2h (backfill; AUTO_RESUBMIT continuations carry WALLTIME=5h):$s"; bash ~/toric-code-nqs/nersc/phase3d_sample_jobs.sh 3' 2>&1 | grep -v "^$"
