...
def load_runs_branched(dirs, sweep, fixed, obs, err=None, tol=1e-9, branch_of=None):
    """Like `load_runs`, but keeps branches (e.g. up/dn chains) separate instead of
    collapsing them to one lowest-energy winner per (L, h). Needed for first-order /
    trivial-trivial cuts, where up and dn are two physically distinct metastable
    branches and the interesting locator is where THEY cross, not which one wins.

    `branch_of(stem) -> str` classifies a run's branch from its filename stem; default
    reads the `_up`/`_dn` chain-link suffix, else "cold". Returns {branch: {L: Curve}}
    (same Curve shape as load_runs; cv.E carries the branch's own energies).
    """
    branch_of = branch_of or (lambda stem: "up" if stem.endswith("_up") else
                               ("dn" if stem.endswith("_dn") else "cold"))
    err = err or ("E_err" if obs == "E0" else obs + "_err")
    best = {}                      # (branch, L, h) -> (E0, y, ye, name)
    for d in dirs:
        for f in sorted(Path(d).glob("*.json")):
            if not _is_final_json(f):
                continue
            j = json.loads(f.read_text())
            c, o = j.get("config"), j.get("observables")
            if not c or not o or o.get("E0") is None or bool(j.get("diverged")):
                continue
            if any(abs(float(c.get(k, 0.0)) - v) > tol for k, v in fixed.items()):
                continue
            y = o.get(obs)
            if y is None or not np.isfinite(y):
                continue
            key = (branch_of(f.stem), int(c["L"]), round(float(c[sweep]), 6))
            row = (float(o["E0"]), float(y), float(o.get(err) if o.get(err) is not None else np.nan), f.stem)
            if key not in best or row[0] < best[key][0]:     # lowest E wins WITHIN a branch
                best[key] = row
    out = {}
    for br, L in sorted({(k[0], k[1]) for k in best}):
        pts = sorted((h, *best[(br, L, h)]) for (bb, LL, h) in best if bb == br and LL == L)
        h = np.array([p[0] for p in pts])
        cv = Curve(L, h, np.array([p[2] for p in pts]), np.array([p[3] for p in pts]),
                   obs, src=";".join(str(d) for d in dirs), names=[p[4] for p in pts])
        cv.E = np.array([p[1] for p in pts])
        out.setdefault(br, {})[L] = cv
    return out


def branch_crossing(h1, y1, h2, y2):
    """Where two branch curves cross: interpolate both onto their common h-overlap and
    root-find y1(h) - y2(h). Returns (h_c, y_c), both nan if the branches don't overlap
    or don't cross within the overlap (e.g. one branch already dominates everywhere)."""
    lo, hi = max(h1.min(), h2.min()), min(h1.max(), h2.max())
    if not (lo < hi):
        return float("nan"), float("nan")
    hh = np.linspace(lo, hi, 400)
    d = np.interp(hh, h1, y1) - np.interp(hh, h2, y2)
    idx = np.where(np.sign(d[1:]) != np.sign(d[:-1]))[0]
    if len(idx) == 0:
        return float("nan"), float("nan")
    f = lambda x: np.interp(x, h1, y1) - np.interp(x, h2, y2)     # noqa: E731
    h_c = brentq(f, hh[idx[0]], hh[idx[0] + 1])
    return float(h_c), float(np.interp(h_c, h1, y1))
...
def load_fm_jsons(dirpath, tags=("bulkR1",)):
    ...
    Curve.src, which records the tag actually used per L)."""
