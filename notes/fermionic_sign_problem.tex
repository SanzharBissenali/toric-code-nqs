...
zero field. The standard 2D fermionic code dresses each \emph{star} with
$\sz$ on the boundary of an adjacent plaquette---say $p(v)$, the plaquette to
the vertex's northeast---while the plaquette family stays untouched (on the
two shared edges $\sx$ and $\sz$ coincide, giving $\sy$ up to phase):
...
\end{tikzpicture}
\qquad\qquad
\raisebox{2.1em}{$\displaystyle \tilde A_v \;=\; A_v\,B_{p(v)},
\qquad B_p \text{ untouched}$}
\end{center}

\noindent
The four $\sx$ form exactly the star of $v$, and the dressing $B_{p(v)}$
\emph{is a stabilizer of the bosonic code}. That has an immediate structural
consequence:
\begin{equation}
\big\langle \tilde A_v,\ \Bp\big\rangle
\;=\;\big\langle A_v B_{p(v)},\ \Bp\big\rangle
...
violation of $\tilde A_v$ is the composite $\epsilon=e\times m$, the fermion
that already existed in the bosonic spectrum, while a lone $\Bp$ violation
remains the flux $m$. (One could equally dress the plaquettes and keep the
stars, $\tilde B_p=A_{v(p)}\Bp$ with $\Av$ untouched: the same group again,
now with elementary excitations $\{e,\epsilon\}$ instead of
$\{\epsilon,m\}$. The two conventions are exchanged by $e\leftrightarrow m$
duality; the operator drawn above belongs to both. Our 3D construction is of
this second, plaquette-dressed kind.) The 2D fTC is the toric code with its
fermion promoted to fundamental charge; the models differ in their
...
language. The rule $\tilde A_v\ket\psi=\ket\psi$ reads
$\psi(s\oplus\mathrm{star}_{v})=\varepsilon_{p(v)}(s)\,\psi(s)$ with
$\varepsilon_p(s)=(-1)^{|\partial p\cap s|}$---formally sign-carrying. But the
Hamiltonian retains the \emph{diagonal} $\Bp$ family, whose $+1$ constraints
confine the ground-state support to even-parity configurations,
$\varepsilon_p\equiv+1$ there; and star flips preserve every boundary parity
(a star shares $0$ or $2$ edges with every $\partial p$, in 2D and 3D alike).
The sign factor exists in the algebra but is never \emph{activated} on the
support.
...
Sec.~\ref{sec:2d}---whose off-diagonal operators flip full stars, and whose
untouched diagonal $\Bp$ family confines the support to even parities---stays
sign-free at $h=0$. The 3D decoration pairs break the argument: a pair
