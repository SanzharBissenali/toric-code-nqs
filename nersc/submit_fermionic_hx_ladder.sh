...
# Each task runs 4 tiers sequentially, matching submit_fermionic_obc_bench.sh's
...
# prefit checkpoint once, gated against the committed L=2 OBC referee; other
# tasks wait for it (see the leader/follower block below) rather than racing
...
run geocnn_fermionic_L2_OBC_hx${HX}_hz0.0_plain \
    --arch GeoCNN --cnn_hidden 4 4 4 $GUARD_OPEN

run gridinv_fermionic_L2_OBC_hx${HX}_hz0.0_k2_asymm \
    $GRIDINV_ARCH $GUARD_OPEN

run gridinv_fermionic_L2_OBC_hx${HX}_hz0.0_k2_anaC_k0 \
    $GRIDINV_ARCH --phase_head_frozen --flux_penalty 0 --init_from "$PREFIT"

run gridinv_fermionic_L2_OBC_hx${HX}_hz0.0_k2_anaC_k6 \
