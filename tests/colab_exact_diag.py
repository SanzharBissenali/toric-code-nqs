...
PARAMS = {
    ...
    "save_vecs": False, # also dump eigenvectors (evals/evecs/meta .npz) for NQS fidelity
                        #   (fidelity.load_ed_vectors, tag 2d-final). Large: ~0.27*k GB at L=2.
}
...
def fermionic_plaquettes(geo, J=1.0):
    ...
    convention to tc3d/fermionic_decoration.py.
    """
    e = np.eye(3)
    out = []
...
def run(params):
    ...
    # Optional: persist eigenvectors for the NQS fidelity guardrail (fidelity.py, kept at tag 2d-final).
    # Basis is arange(2^N) with sigma_i = 1 - 2*bit_i(b), qubit index == bit position —
    # the SAME convention fidelity.spin_configs_from_basis (tag 2d-final) rebuilds, so the ED
