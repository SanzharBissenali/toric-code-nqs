...
3. **Convergence check per point** (from the `.snapshots.json` replay,
   `analysis/scripts/eval_snapshots.py --rounds 4..8`):
...
commented (gates stay `SAVE_FIGS = False`) — the user promotes figures
manually. Exception: the eight committed `phaseB_*` benchmark PNGs are
regenerated by `analysis/scripts/phaseB_figs.py` (bit-exact in the repo venv).
